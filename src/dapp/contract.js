...
export default class Contract {
    ...
    async initialize(callback) {
        ...
        this.web3.eth.getAccounts((error, accts) => {
            ...
            this.flightSuretyData.methods.authorizeContract(this.appAddress).send({from: this.owner}, (error, result) => {
                if(error) {
    ...
    async fund(funds, callback) {
        ...
        self.flightSuretyData.methods
            ...
            .send({ from: payload.funder, value: value}, (error, result) => {
                if (!error){
                    self.flightSuretyData.methods.
                    isAirlineActive(payload.funder).call({ from: payload.funder}, (error, result) => {
                        if(!error){
    ...
    async pay(callback) {
        ...
        self.flightSuretyData.methods.
        withdraw().send({ from: self.accounts[0]}, (error, result) => {
            callback(error, result);
        });
    }
}